...
/**
 * This is a boolean validator. After this class is created, call the testXXXX() methods to perform tests when the
 * validate() method is called. Validate.defineBoolean(myObject==null).testTrue().validate(); If the
 * throwValidationExceptionOnFail() method has been called and if the validate fails then a ValidateException will be
 * thrown. Validate.defineBoolean(myObject==null).testEquals(otherObject != null)
 * .throwValidationExceptionOnFail().validate(); If no test method is called, validate() returns a TRUE.
 *
 * @author G Brown
 */
public final class BooleanValidator extends ObjectValidator<Boolean> {
    ...
    /*
     * A flag indicating if a "false" test will be performed when the validate() method is called.
     */
    private boolean _isTestFalse = false;

    /*
     * A flag indicating if a "true" test will be performed when the validate() method is called.
     */
    ...
    /**
     * A method to mark that a "false" test will be performed when the validate() method is called.
     *
    ...
    /**
     * A method to mark that a "true" test will be performed when the validate() method is called.
     *
     * @return The same BooleanValidator instance. This allows for method chaining.
     */
    ...
    protected boolean validateFalse() {
        ...
        if (this._isTestFalse) {
            ...
            if (this._isValidationExceptionThrownOnFail && !isValid) {
                ObjectValidator.throwValidateException("The value is not false. (value = '" + this._value + "').");
            }
        }
        return (isValid);
    }
    ...
    protected boolean validateTrue() {
        ...
        if (this._isTestTrue) {
            ...
            if (this._isValidationExceptionThrownOnFail && !isValid) {
                ObjectValidator.throwValidateException("The value is not true. (value = '" + this._value + "').");
            }
        }
        return (isValid);
    }
