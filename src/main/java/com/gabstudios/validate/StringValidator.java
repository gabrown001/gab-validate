...
/**
 * This is a String validator. After this class is created, call the testXXXX() methods to perform tests when the
...
public class StringValidator extends ObjectValidator<String> {
    ...
    /*
     * A flag indicating if an "equals no case" test will be performed when the validate() method is called.
     */
    ...
    /*
     * A flag indicating if a "match" will be performed using regrex expressions when the validate() method is called.
     */
    ...
    /**
     * A method to mark that an "equals no case" test will be performed when the validate() method is called.
     *
     * @param equalsValue
     *            The value to perform the equate with.
     *
    ...
    /**
     * A method to mark that an "match" test using regex will be performed when the validate() method is called.
     *
     * @param value
     *            A regular expression that be used to see if the String value is a match.
     *
     * @return The same StringValidator instance. This allows for method chaining.
     */
    ...
    protected boolean validateEqualsNoCase() {
        ...
        if (this._isTestEqualsNoCase) {
            ...
            if (this._isValidationExceptionThrownOnFail && !isValid) {
                ObjectValidator.throwValidateException("The value does not equal the expected value (string value = '"
            ...
            if (this._isValidationExceptionThrownOnFail && !isValid) {
                ObjectValidator
                        .throwValidateException("The value must be greater than or equal to the min value (value = '"
                                + this._value + "' length = '" + ((this._value != null) ? this._value.length() : "null")
                                + "' min value = '" + this._minLength + "').");
            }
    ...
    protected boolean validateMatch() {
        ...
        if (this._isTestMatch) {
            ...
            if (this._isValidationExceptionThrownOnFail && !isValid) {
                ObjectValidator.throwValidateException("The value does not match the reqular expression (value = '"
                        + this._value + "' regex = '" + this._matchValue + "').");
            }
        }
        return (isValid);
    }
