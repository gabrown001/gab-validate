...
/**
 * This is a Object validator. After this class is created, call the testXXXX() methods to perform tests when the
 * validate() method is called. Validate.defineObject(myObject).testNotNull().validate(); If the
 * throwValidationExceptionOnFail() method has been called and if the validate fails then a ValidateException will be
 * thrown. Validate.defineObject(myObject).testEquals(expectedObject).throwValidationExceptionOnFail().validate(); If no
 * test method is called, validate() returns a TRUE.
 *
 * @author G Brown
 *
 * @param <C>
...
public class ObjectValidator<C> implements Validator {
    ...
    /*
     * A flag indicating if an "equals" test will be performed when the validate() method is called.
     */
    protected boolean _isTestEquals = false;

    /*
     * A flag indicating if an "not null" test will be performed when the validate() method is called.
     */
    ...
    protected boolean validateNotNull() {
        ...
        if (this._isTestNotNull) {
            ...
            if (this._isValidationExceptionThrownOnFail && !isValid) {
                ObjectValidator.throwValidateException("The value must not be null");
            }
        }
        return (isValid);
    }
    ...
    protected boolean validateEquals() {
        ...
        if (this._isTestEquals) {
            ...
            if (this._isValidationExceptionThrownOnFail && !isValid) {
                ObjectValidator.throwValidateException("The value does not equal the expected value (value = '"
                        + this._value + "' expected value = '" + this._equalsValue + "').");
            }
        }
        return (isValid);
    }
     ...
     * @param errorMessage The error message to include in an exception if it is created.
     */
    protected static void throwValidateException(final String errorMessage) {
    ...
    /**
     * A method to mark that an IllegalArgumentException should be thrown if the validate method returns false.
     *
     * @param <T>
     *            A type of validator.
     *
