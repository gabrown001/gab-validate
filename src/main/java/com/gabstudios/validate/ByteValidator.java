...
/**
 * This is a byte validator. After this class is created, call the testXXXX() methods to perform tests when the
 * validate() method is called. Validate.defineByte(byte).testNotNull().validate(); If the
 * throwValidationExceptionOnFail() method has been called and if the validate fails then a ValidateException will be
 * thrown. Validate.defineByte(byte).testEquals(expectedByte) .throwValidationExceptionOnFail().validate(); If no test
...
public final class ByteValidator extends NumberValidator<Byte> {
    ...
    private static byte returnNullByte() {
